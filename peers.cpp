...
int client::parse_command(std::string command){
    ...
    // }
        ...
        if (parameters.size() < 2) {
            std::cerr << "Error: Not enough parameters for share command." << std::endl;
            return 00;
        }

        std::stringstream file_number_string(parameters[1]);
        int file_number = 0;
        if (!(file_number_string >> file_number)) {
            std::cerr << "Error: Invalid file number." << std::endl;
            return 0;
        }

        if (parameters.size() < 2 + file_number) {
            std::cerr << "Error: Not enough file names provided." << std::endl;
            return 0;
        }

        std::vector<struct file_info> file_infos;  // Giả sử file_infos là một vector chứa thông tin tệp
        for (int i = 0; i < file_number; i++) {
            struct file_info temp_file_info;
            temp_file_info.file_name = parameters[i + 2];
            temp_file_info.file_length = get_file_size(parameters[i + 2]); // Kiểm tra lại hàm này
            file_infos.push_back(temp_file_info);
        }

        register_request(file_number, file_infos);  // Kiểm tra lại hàm này
    }
